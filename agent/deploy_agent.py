...
This is the always-on deploy daemon only. The batch pipeline (menu
extraction, AI stages, wave submit/poll) has been extracted to
agent/batch_pipeline.py and must be invoked manually as a CLI.
See agent/SEPARATION_PLAN.md for the full separation rationale.
...
def main():
    ...
    print(f"  Mode     : deploy-only (batch pipeline is CLI-only via agent/batch_pipeline.py)")
    print()
