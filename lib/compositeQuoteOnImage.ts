  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        // First line's baseline = top of text block + one quote-font-size
        ...
        if (hasAttribution) {
          ...
          ctx.globalAlpha = 0.7;
          // baselineY is currently one line-height past the last quote line;
          // back off by the unused gap, then add the requested 12px margin + font size.
...
/**
 * Greedy word-wrap against a Canvas 2D context. Splits `text` into lines that
 * each fit within `maxWidth` pixels in the context's currently-configured font.
 */
function wrapText(
